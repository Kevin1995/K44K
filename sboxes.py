...
# THIS IS OUR SUBWORD FUNCTION - WE PASS AN ARRAY OF HEX VALUES AND SUBSTITUTE THEM BASED ON THE sbox TABLE
def encrypt_sbox(hex):
    hex_splitter = []
    for i in hex:
        hex_splitter.append(i)

    for idx, val in enumerate(hex_splitter):
        i, j = 0, 0
        for i in range(17):
            if hex_splitter[idx][0] in sbox[i][0]:
                i = i
                break
        for j in range(17):
            if hex_splitter[idx][1] in sbox[0][j]:
                j = j
                break
        hex_splitter[idx] = sbox[i][j]

    return hex_splitter

# THIS IS OUR INVERSE SUBWORD FUNCTION - WE PASS AN ARRAY OF HEX VALUES AND SUBSTITUTE THEM BASED ON THE inverse_sbox TABLE
def decrypt_sbox(hex):
    #print("aes_table")
    hex_splitter = []
    for i in hex:
        hex_splitter.append(i)
    ...
    for idx, val in enumerate(hex_splitter):
        i, j = 0, 0
        for i in range(17):
            if hex_splitter[idx][0] in inverse_sbox[i][0]:
                i = i
                break
        for j in range(17):
            if hex_splitter[idx][1] in inverse_sbox[0][j]:
                j = j
                break
        hex_splitter[idx] = inverse_sbox[i][j]
